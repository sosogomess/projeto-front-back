...
export default function ColorCard({ color, showFavoriteButton = true }) {
  ...
  useEffect(() => {
    const favorites = JSON.parse(localStorage.getItem('hairToneFavorites') || '[]');
    ...
    <Link href={`/detalhes/${color.id}`} className={styles.colorCard}>
      ...
      <div className={styles.imageContainer}>
        ...
        {showFavoriteButton && (
          ...
          >
            <svg className={styles.favoriteIcon} fill={isFavorite ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
